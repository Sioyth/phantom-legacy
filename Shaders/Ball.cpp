...
Ball::Ball(CameraEditor& camera, float mass, float vMultiplyer, float fMultiplyer)
{
	m_color.r = 1.0f;
	m_color.g = 1.0f;
	m_color.b = 1.0f;
	...
	m_mass = mass;
	m_vMultiplyer = vMultiplyer;
	m_fMultiplyer = fMultiplyer;
...
void Ball::Create()
{
	...
	m_sphereCollider = new SphereCollider(*this);

	m_sphereCollider->SetMass(m_mass);
	m_sphereCollider->AddForce(m_camera->GetForwardVector() * m_fMultiplyer);
	m_sphereCollider->SetVelocity(m_camera->GetForwardVector() * m_vMultiplyer);
